(() => {
  ...
  const MEDIA = {
    ...
    cameraTelephoto: {src:'assets/equipment/field-camera-telephoto.svg', alt:'Weather-sealed field camera with an extra-large telephoto lens', credit:'Original equipment illustration', source:''},
...
state.offers=[]; const specialtyCount=new Set(state.scientists.map(item=>item.specialty)).size,seniorBonus=hiredCareerCount('postdoc')+hiredCareerCount('professor')*2,offerLimit=Math.min(9,Math.max(3,2+specialtyCount+seniorBonus)),usedPictures=new Set();
  ...
  function renderSidebar() {
    ...
    const cool=state.publicationCooldown>0?`${Math.ceil(state.publicationCooldown)} d submission cooldown`:!level?`${Math.ceil(PUBLISH_MIN-state.data)} more data for an Arctic Field Research Note`:level.next?`${level.label} ready · ${Math.round(chance*100)}% acceptance`:'Landmark paper threshold reached · automatic submission';
  ...
  function createRecoveryTarget(deployment) {
    ...
    const target={
      ...
      media:clone(deployment.media),lat:deployment.lat,lon:deployment.lon,data:deployment.data,reward:deployment.reward,upfront:deployment.upfront||0,advancePaid:deployment.advancePaid||0,
