(() => {
  ...
  // legacy validation marker: grants-v7-
  ...
  function missionSpecificSteps(target={}) {
  ...
  function generateOffers(port,{fresh=false}={}) {
    if(!port)return; const portId=normalizedPortId(port),cycle=`${portId}:${state.portVisits}`,legacyGenericFallback=playerCareerLevel()>=2&&(state.offers||[]).some(item=>String(item.templateId||'').startsWith('fallback-')&&!(item.equipment||[]).length); if(!fresh&&state.grantOfferCycle===cycle&&!legacyGenericFallback)return; state.grantOfferCycle=cycle;
...
function vesselOverlay() {
  ...
  function ensureUI() {
    ...
    style.textContent+=`
      ...
      @media(max-width:1200px){.arx-detail-split{grid-template-columns:1fr}.arx-detail-split>.arx-media{height:180px}.arx-store-details>summary{padding:12px}.arx-store-details>summary em{font-size:8px}.arx-vessel-overview>header h2{font-size:25px}}
    `;
    style.textContent+=`
