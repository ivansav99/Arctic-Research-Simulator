(() => {
  ...
  const MEDIA = {
    ...
    aerial: {src:'assets/research/aerial-survey.webp', alt:'Aerial survey image of a polar bear on Chukchi Sea ice', credit:'NOAA Fisheries', source:'https://www.fisheries.noaa.gov/feature-story/us-and-russian-scientists-partner-study-polar-bear-and-seal-populations'},
    storm: {src:'assets/research/storm-sea.webp', alt:'Icebreaker in high Arctic seas', credit:'NOAA', source:'https://response.restoration.noaa.gov/evaluating-oil-spill-response-technologies-arctic'},
    xbt: {src:'assets/equipment/xbt-kit.webp', alt:'XBT launcher and expendable probe case', credit:'Original face-free equipment illustration', source:''},
    argo: {src:'assets/equipment/polar-float.webp', alt:'Polar profiling float secured in a deck cradle', credit:'Original face-free equipment illustration', source:''},
    hydrophone: {src:'assets/equipment/hydrophone-array-face-free.webp', alt:'Towed hydrophone array, cable reel and acquisition case', credit:'Original face-free equipment illustration', source:''},
    sonobuoy: {src:'assets/equipment/sonobuoy-pack.webp', alt:'Four scientific sonobuoys and receiver in a rugged case', credit:'Original face-free equipment illustration', source:''},
    ...
    aerostat: {src:'assets/equipment/aerostat-face-free.webp', alt:'Tethered atmospheric profiling aerostat, instrument pod and winch on an Arctic vessel', credit:'Original face-free equipment illustration', source:''},
    drone: {src:'assets/equipment/large-drone-face-free.webp', alt:'Large fixed-wing scientific drone secured in its Arctic vessel launch cradle', credit:'Original face-free equipment illustration', source:''},
    drifter: {src:'assets/equipment/surface-drifter-pair.webp', alt:'Pair of autonomous surface drifters secured for deployment', credit:'Original face-free equipment illustration', source:''},
    starlink: {src:'assets/equipment/starlink-terminal.webp', alt:'Flat-panel satellite internet terminal on an Arctic vessel', credit:'Original face-free equipment illustration', source:''},
    winch: {src:'assets/equipment/medium-science-winch.webp', alt:'Medium oceanographic winch with a drum of steel instrument wire', credit:'Original face-free equipment illustration', source:''},
    hullSensor: {src:'assets/equipment/hull-echosounder-sensor.webp', alt:'Compact through-hull scientific echosounder transducer and interface', credit:'Original face-free equipment illustration', source:''},
    serviceTools: {src:'assets/equipment/coastal-service-toolkit.webp', alt:'Coastal service hand tools, deck hook, calibration case and replacement hardware', credit:'Original face-free equipment illustration', source:''},
    handheldWater: {src:'assets/equipment/handheld-water-lab.webp', alt:'Rugged handheld multiparameter water-quality meter, probe and sample bottles', credit:'Original face-free equipment illustration', source:''},
    iceCorer: {src:'assets/equipment/ice-corer-auger.webp', alt:'Portable sea-ice corer and powered auger laid out on a sled', credit:'Original face-free equipment illustration', source:''},
    miniRov: {src:'assets/equipment/mini-rov.webp', alt:'Compact observation ROV with tether reel and control case', credit:'Original face-free equipment illustration', source:''},
    shallowAdcp: {src:'assets/equipment/shallow-adcp.webp', alt:'Compact high-frequency four-beam ADCP in a small boat deployment frame', credit:'Original face-free equipment illustration', source:''},
    shelfAdcp: {src:'assets/equipment/shelf-adcp.webp', alt:'Vessel-mounted shelf-depth acoustic Doppler current profiler transducer', credit:'Original face-free equipment illustration', source:''},
    deepAdcp: {src:'assets/equipment/deep-adcp.webp', alt:'Low-frequency deep-water ADCP transducer and electronics', credit:'Original face-free equipment illustration', source:''},
    ek80: {src:'assets/equipment/ek80-scientific-echosounder.webp', alt:'Portable multi-frequency scientific echosounder transceivers and split-beam sensors', credit:'Original face-free equipment illustration', source:''},
    sedimentCorer: {src:'assets/equipment/box-corer.webp', alt:'Oceanographic box corer with open sampling jaws on a vessel deck', credit:'Original face-free equipment illustration', source:''},
    coastalAFrame: {src:'assets/equipment/coastal-a-frame.webp', alt:'Compact stern A-frame and shallow mooring handling rig', credit:'Original face-free equipment illustration', source:''},
  ...
  const CAREERS = {
    grad: {id:'grad', name:'Graduate Student', short:'GRAD', level:1, minCitations:0, salary:300, quality:.88, productivity:1, color:'#8ef0cf', next:'postdoc', promotion:{papers:2,citations:100}},
    ...
    professor: {id:'professor', name:'Professor', short:'PROF', level:3, minCitations:1000, salary:1900, quality:1.42, productivity:1.8, color:'#f6d365', next:null, promotion:null}
  };
  const SLOT_TYPES = ['light', 'medium', 'heavy'];
  const EQUIPMENT_RESALE_RATE = 1;
  const VESSEL_TRADE_IN_RATE = .5;
  const RELOCATION_COST = 10000;
  const RESEARCH_INTERACTION_KM = 10;
  const PAPER_LEVELS = [
    {id:'local',threshold:100,next:1000,label:'Local science newsletter',journal:'Svalbard Science Bulletin',award:30000,initialCitations:5,potential:45},
  ...
  const VESSEL_IMAGES = {
    coastal:'data:image/svg+xml;charset=UTF-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20viewBox%3D%220%200%201200%20520%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id%3D%22sea%22%20x1%3D%220%22%20y1%3D%220%22%20x2%3D%220%22%20y2%3D%221%22%3E%3Cstop%20offset%3D%220%22%20stop-color%3D%22%23123f56%22%2F%3E%3Cstop%20offset%3D%221%22%20stop-color%3D%22%2308293b%22%2F%3E%3C%2FlinearGradient%3E%3ClinearGradient%20id%3D%22hull%22%20x1%3D%220%22%20y1%3D%220%22%20x2%3D%221%22%20y2%3D%221%22%3E%3Cstop%20stop-color%3D%22%23f8fbfa%22%2F%3E%3Cstop%20offset%3D%221%22%20stop-color%3D%22%23d5e5e7%22%2F%3E%3C%2FlinearGradient%3E%3C%2Fdefs%3E%0A%3Crect%20width%3D%221200%22%20height%3D%22520%22%20rx%3D%2234%22%20fill%3D%22url%28%23sea%29%22%2F%3E%3Cpath%20d%3D%22M0%20415%20Q280%20388%20600%20414%20T1200%20408%20V520H0Z%22%20fill%3D%22%23245d72%22%20opacity%3D%22.7%22%2F%3E%0A%3Cg%20transform%3D%22translate%2870%2054%29%22%3E%3Cpath%20d%3D%22M90%20319%20L917%20319%20L1045%20280%20L982%20374%20L176%20374%20Z%22%20fill%3D%22%23183b50%22%20stroke%3D%22%236aa7b6%22%20stroke-width%3D%226%22%2F%3E%3Cpath%20d%3D%22M168%20247%20H846%20L935%20319%20H118%20Z%22%20fill%3D%22url%28%23hull%29%22%20stroke%3D%22%2386b6c1%22%20stroke-width%3D%226%22%2F%3E%3Cpath%20d%3D%22M235%20164%20H678%20L744%20247%20H194%20Z%22%20fill%3D%22%23f8fbfa%22%20stroke%3D%22%237ba9b5%22%20stroke-width%3D%226%22%2F%3E%3Cpath%20d%3D%22M333%20103%20H583%20L638%20164%20H292%20Z%22%20fill%3D%22%23f8fbfa%22%20stroke%3D%22%237ba9b5%22%20stroke-width%3D%226%22%2F%3E%3Cg%20fill%3D%22%233e88a4%22%3E%3Crect%20x%3D%22329%22%20y%3D%22126%22%20width%3D%2246%22%20height%3D%2224%22%20rx%3D%224%22%2F%3E%3Crect%20x%3D%
D%22274%22%20width%3D%2273%22%20height%3D%2248%22%20rx%3D%225%22%20fill%3D%22%23d79d43%22%2F%3E%3Crect%20x%3D%22653%22%20y%3D%22274%22%20width%3D%2271%22%20height%3D%2248%22%20rx%3D%225%22%20fill%3D%22%235f9ab1%22%2F%3E%3Cpath%20d%3D%22M77%20333%20Q510%20381%20996%20339%22%20fill%3D%22none%22%20stroke%3D%22%23671722%22%20stroke-width%3D%2212%22%2F%3E%0A%3C%2Fg%3E%3C%2Fsvg%3E'
  };
  const VESSELS = {
    fishing: {
      id:'fishing', name:'Small Fishing Vessel', shipName:'F/V Isfjord', className:'LOCAL CLASS', price:0, berths:3, image:'assets/vessels/fishing-vessel.webp',
    ...
    trawler: {
      id:'trawler', name:'Larger Fishing Trawler', shipName:'F/V Nordlys', className:'EXPEDITION TRAWLER', price:280000, berths:5, image:'assets/vessels/fishing-trawler.webp',
    ...
    icebreaker: {
      id:'icebreaker', name:'Basic Icebreaker', shipName:'R/V Borealis', className:'ICEBREAKER', price:60000000, berths:30, image:VESSEL_IMAGES.icebreaker,
  ...
  const TEMPLATES = [
    ...
    mission({id:'duck-count', tier:'local', title:'Spitsbergen Duck Count', shortTitle:'DUCK COUNT', anyScientist:true, equipment:['field-optics'], data:4, reward:3000, supplies:1, workHours:12, minDistance:8, distanceRange:25, coastal:true, transect:true, stationCounts:{fishing:3,trawler:4,coastal:5,global:6,icebreaker:7,nuclear:8}, stationSpacingKm:4, media:MEDIA.aerial, description:'Count summer waterfowl along a short Spitsbergen shoreline transect using stabilized field optics.', steps:['Review identification sheet','Set a slow shoreline track','Count birds by transect segment','Photograph uncertain groups','Submit the observation sheet']}),
    ...
    mission({id:'shallow-mooring-deploy', title:'Deploy a Shallow Coastal Mooring', shortTitle:'COASTAL MOORING', specialties:['moorings','coastal-oceanography'], equipment:['shallow-mooring-kit','coastal-a-frame'], data:38, reward:62000, supplies:10, workHours:54, missionMode:'mooring-deploy', recoveryAfterDays:14, coastal:true, media:MEDIA.drifter, description:'Deploy a recoverable coastal mooring for a short current and temperature time series, then return after two weeks.', steps:['Inspect releases and sensors','Rig the coastal A-frame','Stream the mooring line','Set the anchor on position','Range the release and log deployment']}),
    ...
    mission({id:'coastal-ecosystem', title:'Nearshore Biodiversity Survey', shortTitle:'COASTAL ECOLOGY', specialties:['coastal-ecology'], equipment:['edna-lab'], data:30, reward:36000, supplies:10, workHours:52, media:MEDIA.local, coastal:true, description:'Combine eDNA, shoreline observations and habitat samples in a nearshore ecosystem.', steps:['Document shoreline habitat','Prepare sterile filtration gear','Collect replicate eDNA samples','Preserve filters and field blanks','Complete the biodiversity log']}),
    mission({id:'ice-physics', title:'Drifting Ice-Floe Station', shortTitle:'ICE STATION', specialties:['sea-ice-physics','sea-ice-ecology'], equipment:['ice-core-system'], data:34, reward:42000, supplies:12, workHours:66, media:MEDIA.ice, iceAllowed:true, description:'Characterize sea ice, snow and the under-ice environment from a suitable floe.', steps:['Assess floe safety and drift','Lay out the ice transect','Drill thickness and snow stations','Collect ice cores and light readings','Return equipment and samples aboard']}),
    mission({id:'plankton-bloom', title:'Plankton Bloom Survey', shortTitle:'PLANKTON BLOOM', specialties:['plankton'], equipment:['plankton-winch'], data:34, reward:39000, supplies:10, workHours:54, media:MEDIA.local, description:'Sample a concentrated plankton feature and connect it to the surrounding water column.', steps:['Map fluorescence around the bloom','Prepare nets and imaging system','Tow through the target layer','Image and preserve subsamples','Clean the net and close the station']}),
    ...
    mission({id:'mooring-deploy', title:'Deploy Long-Term Arctic Mooring', shortTitle:'MOORING DEPLOY', specialties:['moorings','physical','naval-acoustics'], equipment:['deep-mooring-payload','heavy-winch','stern-a-frame'], data:78, reward:155000, supplies:24, workHours:132, missionMode:'mooring-deploy', recoveryAfterDays:70, recoveryAfterDaysByVessel:{global:70,icebreaker:180,nuclear:365}, media:MEDIA.drifter, description:'Deploy a composite physical and acoustic mooring. The full time series and contract value arrive only after a later recovery.', steps:['Inspect releases and payload stack','Stream the instrument line astern','Lower flotation and sensors','Drop the anchor on position','Range the release and log deployment']}),
    ...
    mission({id:'aerial-wildlife', title:'Aerial Ice & Wildlife Survey', shortTitle:'AERIAL SURVEY', specialties:['marine-mammals','sea-ice-physics'], equipment:['large-drone'], transect:true, stationCounts:{icebreaker:5,nuclear:8}, stationSpacingKm:45, data:52, reward:84000, supplies:10, workHours:84, media:MEDIA.aerial, description:'Fly thermal and visible cameras across connected ice-habitat corridors to map wildlife and surface conditions.', steps:['File the survey corridor','Check aircraft and payload','Launch from the helideck','Fly the thermal-image leg','Recover before the next corridor']}),
    mission({id:'drone-ice-edge', title:'Long-Range Drone Ice-Edge Reconnaissance', shortTitle:'ICE RECON', specialties:['sea-ice-physics','atmosphere'], equipment:['large-drone'], transect:true, stationCounts:{icebreaker:5,nuclear:9}, stationSpacingKm:55, data:56, reward:91000, supplies:9, workHours:78, iceAllowed:true, media:MEDIA.drone, description:'Map leads, ridging and low-cloud structure along a long ice-edge corridor with a deck-launched drone.', steps:['Plan the ice-edge flight','Inspect airframe and sensors','Launch from the helideck','Map leads and cloud base','Recover and mosaic the imagery']}),
  ...
  const PLAYER_AVATARS = [
    {id:'chief-1',src:'assets/scientists/maya-chen.webp',label:'Teal parka'},
    {id:'chief-2',src:'assets/scientists/noah-okafor.webp',label:'Blue parka'},
    {id:'chief-3',src:'assets/scientists/amara-singh.webp',label:'Red parka'},
    {id:'chief-4',src:'assets/scientists/nuka-petersen.webp',label:'Green parka'},
    {id:'chief-5',src:'assets/scientists/hana-suzuki.webp',label:'Yellow parka'},
    {id:'chief-6',src:'assets/scientists/owen-clarke.webp',label:'Navy parka'}
  ];

  const initialScientist = {id:'player', profileId:null, name:'You', role:'Chief Scientist', specialty:'coastal-oceanography', career:'grad', portrait:PLAYER_AVATARS[0].src, isPlayer:true, recruitmentPool:'player', hiredAt:-1, missions:0, papers:0};
  const state = {
    ...
    observed:[], observedIndividuals:[], claimedGroups:[], papers:[], publicationCooldown:0, publishAttempts:0,
    economyDays:0, elapsedDays:0, log:['Expedition commissioned in Longyearbyen.'], navigation:null, lastTargetContext:null,
    scientistRecords:{}, promotions:[], recentGrantTemplates:[], recentGrantSites:[], grantCooldowns:{}, grantMarketReady:{}, assistedByVessels:[], bridgeSupportNotice:null, lastPortId:null, lastProfessorGrantDay:-999, remoteOffer:null
  };
  ...
  let characterDraft = {avatar:PLAYER_AVATARS[0].id,specialty:'coastal-oceanography'};
  ...
  function careerHireStatus(id) {
    if (id==='grad') return {ready:true,label:'Graduate students are not citation-limited'};
  ...
  function grantCapacity() { return Math.max(1,state.scientists.length); }
  function grantLoad() {
  ...
  function vesselsForPort(port=state.port) {
    const id=normalizedPortId(port);
    if (id==='murmansk') return [VESSELS.nuclear];
    if (id==='longyearbyen') return ['fishing','trawler','coastal','global','icebreaker'].map(id=>VESSELS[id]);
    return [];
  }
  function vesselForSaleHere(id,port=state.port) {
    const portId=normalizedPortId(port);
    if (id==='nuclear') return portId==='murmansk';
    return portId==='longyearbyen' && ['fishing','trawler','coastal','global','icebreaker'].includes(id);
  }
  ...
  function checkPromotions() {
    ...
    if (player?.career==='grad' && state.papers.length>=2 && state.citations>=100) {
      player.career='postdoc'; recordScientist(player); promotionQueue.push({name:'Chief Scientist',career:'postdoc',papers:state.papers.length,missions:player.missions||0,message:'Two published papers and 100 citations have earned postdoc status. You may now hire postdocs (one per 100 citations), purchase medium-duty science systems, commission a coastal-class research vessel, and receive much more sophisticated postdoc-level research programs.'}); addLog('Chief Scientist promoted to postdoc · coastal R/V and medium equipment unlocked.');
    }
  ...
  function slotSummary(ship=vessel(), usage=slotUsage()) {
    return SLOT_TYPES.map(type => `${type[0].toUpperCase()}${type.slice(1)} ${usage[type]}/${ship.slots[type]}`).join(' · ');
  }
  ...
  function adjustMoney(delta,{sound=true}={}) {
    ...
    const from=state.money; state.money+=delta; cashAnimation={from,to:state.money};
  ...
  function buildTarget(template, origin, rng, kind='grant', options={}) {
    ...
    const target={
      ...
      data:Math.max(1,Math.round(template.data*scale*vesselScale*crewScale*iceValueMultiplier)), reward:Math.round(template.reward*scale*(kind === 'opportunity' ? 1.6 : 2)*iceValueMultiplier),
      ...
      anywhere:!!template.anywhere, stationDelivery:!!template.stationDelivery, berthReserve:template.berthReserve||0, shore:!!template.shore, glacier:!!template.glacier, terrestrial:!!template.terrestrial, fjordPreferred:!!template.fjordPreferred, siteName:point.siteName||null, iceValueMultiplier, status:'active', kind, selected:false,
  ...
  function generateOffers(port,{fresh=false}={}) {
    ...
    const available=TEMPLATES.filter(item=>!item.weather&&eligible(item)&&!activeTemplates.has(item.id)&&!(state.droppedGrantTemplates||[]).includes(item.id)&&(item.unlockAfter||0)<=state.completed.length&&(!item.onlyPorts||item.onlyPorts.includes(portId))&&(state.grantCooldowns?.[`${portId}:${item.id}`]||0)<=state.elapsedDays);
    const teamLevel=Math.max(1,...state.scientists.map(item=>careerLevel(item.career))),postdocCount=state.scientists.filter(item=>item.career==='postdoc').length,professorCount=state.scientists.filter(item=>item.career==='professor').length,weighted=[];
    ...
    state.offers=[]; const specialtyCount=new Set(state.scientists.map(item=>item.specialty)).size,seniorBonus=hiredCareerCount('postdoc')+hiredCareerCount('professor')*2,offerLimit=Math.min(9,Math.max(3,2+specialtyCount+seniorBonus));
    for(const template of pool){if(state.offers.length>=offerLimit)break;const target=buildTarget(template,port,rng,'grant');if(target)state.offers.push(target);} if(!state.offers.length){const fallback=buildTarget(compatibleFallbackTemplate(),port,rng,'grant');if(fallback)state.offers.push(fallback);}
  }

  function mediaMarkup(item, className='') {
    const media=item?.media;
    if (!media?.src) return '';
    return `<figure class="arx-media ${className}"><img src="${escapeHtml(media.src)}" alt="${escapeHtml(media.alt || item.name || item.title)}"><figcaption>${escapeHtml(media.credit || '')}${media.source?` · <a href="${escapeHtml(media.source)}" target="_blank" rel="noopener">source</a>`:''}</figcaption></figure>`;
  }
  function scientistCard(item,candidate=false) {
    ...
    const capability=career.level===1?'LIGHT EQUIPMENT':career.level===2?'LIGHT + MEDIUM EQUIPMENT':'LIGHT + MEDIUM + HEAVY EQUIPMENT',selected=candidate&&pendingCandidateId===item.id; let action;
  ...
  function vesselTradeInValue(ship=vessel()) {
    const hull=ship.id==='fishing'?60000:Math.round((ship.price||0)*VESSEL_TRADE_IN_RATE);
  ...
  function equipmentCard(item) {
    ...
    const tier=['','GRAD','POSTDOC','PROFESSOR'][item.tier] || 'FIELD';
  ...
  function missionReadiness(target) {
    const projection=missionFoodProjection(target), rows=[];
    ...
    for (const id of target.consumables||[]) {
      const needed=remainingConsumableNeed(target,id), aboard=state.inventory[id]||0;
      rows.push({label:EQUIPMENT[id]?.name||id,ready:aboard>=needed,detail:`${aboard} aboard · ${needed} needed for remaining stations`});
    }
    const supplies=remainingSupplies(target);
    rows.push({label:'Lab supplies',ready:state.supplies>=supplies,detail:`${state.supplies} aboard · ${supplies} needed`});
    rows.push({label:'Food through the work and return',ready:projection.remaining>=15,detail:`Projected ${Math.max(0,Math.floor(projection.remaining))}% on return`});
    if (!vessel().nuclearFuel) rows.push({label:'Fuel through the work and return',ready:projection.fuelRemaining>=10,detail:`Projected ${Math.max(0,Math.floor(projection.fuelRemaining))}% on return`});
  ...
  function offerCard(item) {
    const specialty=item.anyScientist?'Any scientist aboard':item.specialties.map(id=>specialtyById[id]?.name).filter(Boolean).join(' / ');
    ...
    return `<article class="arx-card offer"><div class="arx-offer-thumb"><img src="${escapeHtml(item.media?.src||MEDIA.local.src)}" alt=""></div><div class="arx-card-head"><div><b>${escapeHtml(item.title)}</b><small>${escapeHtml(specialty)}</small></div><em>${cash(item.reward)}</em></div><p>${escapeHtml(item.description)}</p><div class="arx-stats"><span>+${item.data} data</span><span>${item.supplies} supplies</span><span>${item.workHours} person-hours</span><span>~${projection.days} field days</span>${item.iceValueMultiplier>1?`<span>ICE PREMIUM ×${item.iceValueMultiplier.toFixed(2)}</span>`:''}<span>Food on return ~${Math.max(0,Math.floor(projection.remaining))}%</span><span>Fuel on return ~${Math.max(0,Math.floor(projection.fuelRemaining))}%</span></div><button data-arx-action="accept" data-id="${item.id}" ${cap||foodUnsafe||fuelUnsafe?'disabled':''}>${label}</button></article>`;
  }
  function activeGrantCard(item) {
    const missing=!eligible(item), projection=missionFoodProjection(item);
    const recovery=!!item.deploymentId;
  ...
  function collectingGrantCard(item) {
    return `<article class="arx-card grant"><div class="arx-card-head"><div><b>${escapeHtml(item.title)}</b><small>INSTRUMENT COLLECTING · RETURN VISIT PENDING</small></div><em>${Math.max(0,Math.ceil(item.remainingDays||0))} d</em></div><p>Autonomous observations are underway. This research grant continues to occupy one scientist-led grant slot until recovery.</p><button disabled>RECOVERY WINDOW NOT OPEN YET</button></article>`;
  }
  ...
  function renderPort() {
    ...
    const tab=(id,label,attention=false)=>`<button data-arx-tab="${id}" class="${activePortTab===id?'active':''} ${attention?'attention':''}">${label}</button>`, panel=id=>`arx-tab ${activePortTab===id?'active':''}`;
    ...
    modal.innerHTML=`<div class="arx-modal-card arx-port-card"><button class="arx-close" data-arx-action="close-port" aria-label="Close port">×</button><header><small>PORT CALL · ${escapeHtml(state.port?.name||'ARCTIC PORT')}</small><h2>Expedition Services</h2></header><div class="arx-port-navrow"><nav class="arx-tabs arx-tabs-top">${tab('vessel','Your Vessel')}${tab('fleet','Shipyard')}${tab('crew','Scientists')}${tab('equipment','Equipment')}${tab('contracts','Research Grants',grantAttention)}${playerCareerLevel()>=2?tab('relocate','Relocate Home Port'):''}</nav><div class="arx-port-cash"><small>CASH</small><b data-arx-cash>${cash(state.money)}</b></div></div>${state.bridgeSupportNotice?`<div class="arx-bridge-support"><b>UNIVERSITY BRIDGE SUPPORT</b><span>${escapeHtml(state.bridgeSupportNotice)}</span></div>`:''}<section class="${panel('vessel')}" data-arx-panel="vessel">${portVesselDashboardMarkup(resources,ship,quote)}</section><section class="${panel('fleet')}" data-arx-panel="fleet"><h3>${normalizedPortId()==='murmansk'?'Murmansk Nuclear Shipyard':normalizedPortId()==='longyearbyen'?'Longyearbyen Vessel Broker':'No vessel dealer at this port'}</h3><p class="arx-help">Every vessel class sold here is displayed from the start. Red prices and the requirement checklist show what still stands between you and the next hull.</p><div class="arx-grid arx-store-list">${vesselsForPort().map(vesselCard).join('')||'<div class="arx-empty"><b>No vessels sold at this port.</b></div>'}</div></section><section class="${panel('crew')}" data-arx-panel="crew"><p class="arx-help">Salaries are deducted daily for everyone aboard, including you. Released scientists remain available to rehire during the same port call.</p><div class="arx-vessel-columns"><section><h3>Available to hire</h3><div class="arx-grid arx-store-list">${state.candidates.map(item=>scientistCard(item,true)).join('')||'<div class="arx-empty"><b>No new candidates at this port.</b></div>'}</div></section><section><h3>Current crew · ${state.scientists.length}/${ship.berths}</h3><div class="arx-grid arx-store-list">${state.scientists.map(item=>scientistCard(item)).join('')}</div></section></div></section><section class="${panel('equipment')}" data-arx-panel="equipment"><h3>Scientific instrumentation for ${escapeHtml(ship.shipName||ship.name)}</h3><p class="arx-help">${slotSummary(ship,usage)} · helidecks ${helideckUsage()}/${ship.helidecks}. Equipment that loses its qualified operator is marked in red until the team is restored.</p>${equipmentCatalogMarkup(ship)}</section><section class="${panel('contracts')}" data-arx-panel="contracts"><h3>Active research grants · ${grantLoad()}/${grantCapacity()}</h3><p class="arx-help">More scientific specialties aboard produce a broader grant board. A different port always has its own fresh market; only return visits to the same port use cooldowns. Ice-capable expeditions receive strong sponsor premiums for work in thicker pack ice.</p><div class="arx-grid">${[...activeGrants().map(activeGrantCard),...collecting.map(collectingGrantCard)].join('')||'<div class="arx-empty"><b>No active research grants.</b></div>'}</div><h3>New grant offers</h3><div class="arx-grid">${state.offers.map(offerCard).join('')||'<div class="arx-empty"><b>Building a compatible sponsor call…</b><p>The grant board always provides at least one program that matches the expertise aboard.</p></div>'}</div></section>${playerCareerLevel()>=2?`<section class="${panel('relocate')}" data-arx-panel="relocate"><h3>Relocate home port</h3>${relocationPanelMarkup()}</section>`:''}</div>`;
  ...
  function renderSidebar() {
    ...
    const cool=state.publicationCooldown>0?`${Math.ceil(state.publicationCooldown)} d submission cooldown`:!level?`${Math.ceil(PUBLISH_MIN-state.data)} more data for a local science paper`:level.next?`${level.label} ready · ${Math.round(chance*100)}% acceptance`:'International paper threshold reached · automatic submission';
    ...
    root.querySelector('#arx-sidebar').innerHTML=`<div class="arx-side-head"><div><small>RESEARCH PROGRAM</small><b data-arx-cash>${cash(state.money)}</b></div><button data-arx-action="toggle-side" aria-label="Close research panel">×</button></div><div class="arx-metrics"><span><small>CITATIONS</small><b>${Math.floor(state.citations)}</b></span><span><small>LAB SUPPLIES</small><b>${state.supplies}/${ship.supplyCapacity}</b></span><span><small>GRANTS</small><b>${grantLoad()}/${grantCapacity()}</b></span></div><section class="arx-data"><div><small>PUBLICATION DATA</small><b>${Math.round(state.data)}</b></div><i class="arx-nonlinear-gauge"><em style="width:${readiness}%"></em><u style="left:12%"></u><u style="left:50%"></u></i><div class="arx-gauge-labels"><span>100 LOCAL</span><span>1,000 NATIONAL</span><span>10,000 INTERNATIONAL</span></div><p>${escapeHtml(cool)}${next?` · next threshold ${new Intl.NumberFormat().format(next.threshold)} data`:''}</p><button class="publish" data-arx-action="publish" ${!level||state.publicationCooldown>0||!level.next?'disabled':''}>${level&&!level.next?'AUTO-PUBLISH READY':'PUBLISH PAPER'}</button></section>${papers.length?`<section class="arx-papers"><small>PUBLICATIONS · CITATIONS GROW DAILY</small>${papers.map(paper=>`<div><span>${escapeHtml(paper.title)}<em>${escapeHtml(paper.journal)} · ${Math.floor(paper.ageDays||0)} d old</em></span><b>${paper.citations||0}</b></div>`).join('')}</section>`:''}${deploymentSummary()}${nav?`<button class="arx-nav ${navOpportunity?'opportunity':''}" data-arx-action="open-nav"><span class="${navOpportunity?'arx-question':'arx-arrow'}" ${navOpportunity?'':`style="transform:rotate(${nav.bearingDeg}deg)"`}>${navOpportunity?'?':'↑'}</span><div><small>${navOpportunity?'DISCOVERED RESEARCH OPPORTUNITY':'ACTIVE RESEARCH GRANT'}</small><b>${escapeHtml(nav.target?.shortTitle||nav.target?.title||'RESEARCH')}</b><em>${Math.round(nav.distanceKm)} km${navOpportunity?'':` · ${Math.round(nav.bearingDeg)}°`}</em></div></button>`:'<div class="arx-nav empty"><span>＋</span><div><small>NO ACTIVE SITE</small><b>Explore or visit a port for grants</b></div></div>'}<div class="arx-side-actions"><button data-arx-action="field-guide">FIELD GUIDE · ${state.observed.length}/${Object.keys(catalog).length}</button><button data-arx-action="open-port" ${state.port?'':'disabled'}>PORT SERVICES</button></div><footer>${escapeHtml(ship.shipName||ship.name)} · ${state.scientists.length}/${ship.berths} berths · payroll ${cash(payroll())}/day</footer>`;
    animateCashReadouts();
  }
  ...
  function enterPort(port,options={}) {
    ...
    if (!options.resume) {
      ...
      const portId=normalizedPortId(state.port),readyAt=state.grantMarketReady?.[portId]||0;
      state.grantOfferCycle=null; generateOffers(state.port,{fresh:true});
      if (differentPort||state.elapsedDays>=readyAt) state.grantMarketReady[portId]=state.elapsedDays+7;
  ...
  function openTarget(id,context={}) {
    ...
    if (activeOperation) { toast('RESEARCH STATION ALREADY IN PROGRESS'); return false; }
    const distance=context.distanceKm ?? (state.navigation?.id===id?state.navigation.distanceKm:Infinity);
    if (distance>RESEARCH_INTERACTION_KM && !target.anywhere) {
      state.targets.forEach(item=>item.selected=item.id===id);
      callbacks.onNavigate?.(target);
      renderSidebar();
      return false;
    }
    state.targets.forEach(item=>item.selected=item.id===id);
    state.lastTargetContext={...context,id,distanceKm:distance};
    ...
    modal.innerHTML=`<div class="arx-modal-card arx-target-card"><button class="arx-close" data-arx-action="close-target" aria-label="Close research site">×</button><small>${target.weather?'LIVE WEATHER RESEARCH':target.kind==='opportunity'||target.kind==='weather-opportunity'?'DISCOVERED RESEARCH OPPORTUNITY:':'ACTIVE RESEARCH GRANT'}</small><h2>${escapeHtml(target.title)}</h2><p>${escapeHtml(target.description)}</p><div class="arx-target-facts compact"><span><small>STATION</small><b>${stationLabel}</b></span><span><small>REMAINING WORK</small><b>${Math.round(remainingWorkHours(target))} person-hours · ~${projection.days} days</b></span><span><small>RESULT</small><b>${['mooring-deploy','staged-deploy','autonomous'].includes(target.missionMode)?'Data after telemetry / recovery':`+${target.data} data`}</b></span></div><h3 class="arx-check-title">Mission readiness</h3>${readinessMarkup(readiness)}<div class="arx-modal-actions single"><button data-arx-action="complete-target" data-id="${target.id}" ${!readiness.ready?'disabled':''}>${escapeHtml(actionLabel)}</button></div></div>`;
    modal.classList.add('open'); renderSidebar(); return true;
  }
  function operationEquipmentMarkup(target) {
    ...
    if (!items.length) return '<div class="arx-operation-equipment"><div class="arx-operation-gear empty"><b>FIELD KIT</b><span>No specialized equipment required</span></div></div>';
    return `<div class="arx-operation-equipment">${items.map(item=>`<div class="arx-operation-gear"><img src="${escapeHtml(item.media?.src||MEDIA.local.src)}" alt=""><span>${escapeHtml(item.name)}</span></div>`).join('')}</div>`;
  }
  ...
  function renderOperationShell(target) {
    const modal=root.querySelector('#arx-target-modal'), steps=activeOperation.steps, station=currentStation(target);
    modal.innerHTML=`<div class="arx-modal-card arx-target-card arx-operation"><small>RESEARCH STATION IN PROGRESS${station?` · STATION ${station.number}/${target.stations.length}`:''}</small><h2>${escapeHtml(target.title)}</h2><h3 class="arx-operation-subhead">QUALIFIED SCIENTISTS</h3>${operationScientistsMarkup(target)}<h3 class="arx-operation-subhead">EQUIPMENT IN USE</h3>${operationEquipmentMarkup(target)}<div class="arx-operation-progress"><div><b id="arx-operation-percent">0%</b><span>${Math.round(activeOperation.workHours)} person-hours · ~${activeOperation.days} game days</span></div><i><em id="arx-operation-bar"></em></i></div><ol>${steps.map((step,index)=>`<li data-arx-step="${index}"><i>${index+1}</i><b>${escapeHtml(step)}</b><span>Queued</span></li>`).join('')}</ol><div class="arx-operation-actions"><button disabled data-arx-operation-state>RESEARCH IN PROGRESS</button><button disabled data-arx-action="acknowledge-research">OKAY</button></div></div>`;
    modal.classList.add('open');
  }
  function participantIdsFor(target) {
    const requiredLevel=Math.max(1,...[...(target.equipment||[]),...(target.consumables||[])].map(id=>EQUIPMENT[id]?.tier||1));
    if (target.anyScientist) return state.scientists.filter(item=>careerLevel(item.career)>=Math.min(requiredLevel,1)).map(item=>item.id);
    return state.scientists.filter(item=>(target.specialties||[]).includes(item.specialty)&&careerLevel(item.career)>=requiredLevel).map(item=>item.id);
  }
  ...
  function completionModal(target,title,body,stats=[]) {
    const modal=root.querySelector('#arx-target-modal');
    const steps=target.steps?.length?target.steps:['Hold the science station','Calibrate instruments','Collect observations','Check sample metadata','Secure the station'];
    modal.innerHTML=`<div class="arx-modal-card arx-target-card arx-operation arx-complete"><small>FIELD PROGRAM UPDATE</small><h2>${escapeHtml(title)}</h2><p>${escapeHtml(body)}</p><div class="arx-operation-progress"><div><b>100%</b><span>${Math.round(target.lastOperationHours||operationWorkHours(target))} person-hours completed at this station</span></div><i><em style="width:100%"></em></i></div><ol>${steps.map((step,index)=>`<li class="done"><i>✓</i><b>${escapeHtml(step)}</b><span>Complete</span></li>`).join('')}</ol><div class="arx-operation-result-space">${stats.length?`<div class="arx-chance">${stats.map(item=>`<span>${escapeHtml(item.label)}<b>${escapeHtml(item.value)}</b></span>`).join('')}</div>`:''}</div><div class="arx-operation-actions"><button disabled data-arx-operation-state>RESEARCH COMPLETE</button><button data-arx-action="acknowledge-research">OKAY</button></div></div>`;
    modal.classList.add('open');
  }
  ...
  function finishOperation(target) {
    ...
    if (['mooring-deploy','staged-deploy'].includes(target.missionMode)) {
      ...
      state.deployments.push({id:`deployment-${target.id}`,title:target.title.replace(/^(Deploy|Deliver) /,''),lat:target.lat,lon:target.lon,status:'collecting',remainingDays:wait,recoveryRequired:true,data:dataGain,reward:target.reward,equipment:[...target.equipment],specialties:[...target.specialties],anyScientist:target.anyScientist,berthReserve:target.berthReserve||0,workHours:Math.max(6,Math.round(target.workHours*.72)),supplies:Math.max(2,Math.round(target.supplies*.55)),media:clone(target.media),recoveryTargetId:null,originalKind:target.kind,templateId:target.templateId,recoveryMode:staged?'staged-recovery':'mooring-recovery',recoveryTitle:target.recoveryTitle,recoveryShortTitle:target.recoveryShortTitle,recoveryDescription:target.recoveryDescription});
      ...
      state.deployments.push({id:`deployment-${target.id}`,title:target.title,lat:target.lat,lon:target.lon,status:'collecting',remainingDays:target.deploymentDays,recoveryRequired:false,data:dataGain,reward:target.reward,equipment:[...target.equipment],specialties:[...target.specialties],media:clone(target.media)});
  ...
  function completeTarget(id,context={}) {
    ...
    const workHours=operationWorkHours(target), days=effectiveDays(target,workHours), durationMs=clamp(5000+days*850,6000,11500);
  ...
  function createRecoveryTarget(deployment) {
    ...
    const target={
      ...
      media:clone(deployment.media),lat:deployment.lat,lon:deployment.lon,data:deployment.data,reward:deployment.reward,
  ...
  function maybeSpawnOpportunity(payload={}) {
    ...
    if (weather?.type && weather.type!=='clear' && weather.eventId && !state.weatherEventsSeen.includes(weather.eventId)) {
      const weatherTemplates=TEMPLATES.filter(item=>item.weather===weather.type), basic=weatherTemplates.find(item=>item.anyScientist), advanced=weatherTemplates.filter(item=>!item.anyScientist&&eligible(item,weather));
      const spawned=[];
      for (const template of [basic,advanced[0]].filter(Boolean)) {
        const rng=seeded(`${weather.eventId}-${template.id}`), target=buildTarget(template,payload.position,rng,'weather-opportunity');
        if (target) { target.selected=spawned.length===0; state.targets.push(target); spawned.push(target); }
      }
  ...
  function openWildlife(species,context={}) {
    ...
    if (firstIndividual) { state.observedIndividuals.push(individualId); addData(2); addLog(`${item.displayName} observation archived · +2 data.`); }
    ...
    modal.innerHTML=`<div class="arx-modal-card arx-wildlife-card"><button class="arx-close" data-arx-action="close-wildlife">×</button><div class="arx-photo ${item.photoFit==='contain'?'contain':''} ${tone}"><img src="${escapeHtml(item.photo)}" alt="${escapeHtml(item.displayName)}"><span>${firstIndividual?'OBSERVATION ARCHIVED · +2 DATA':'THIS INDIVIDUAL ALREADY OBSERVED · +0 DATA'}</span></div><div class="arx-species"><small>${escapeHtml(item.group)} · ${progress.seen}/${progress.total}</small><h2>${escapeHtml(item.displayName)}</h2><em>${escapeHtml(item.scientificName)}</em><p class="arx-observation-note">${firstIndividual?'A new individual observation added exactly 2 data points. Its glowing chart ring is now cleared.':'This is the same animal or school already recorded during this expedition, so no additional data were added.'}</p><ul>${item.facts.map(fact=>`<li>${escapeHtml(fact)}</li>`).join('')}</ul><a href="${escapeHtml(item.source)}" target="_blank" rel="noopener">${escapeHtml(item.credit)}</a><div class="arx-modal-actions"><button data-arx-action="field-guide">OPEN FIELD GUIDE</button><button class="ghost" data-arx-action="close-wildlife">DISMISS</button></div></div></div>`;
  ...
  function release(id) {
    ...
    if (grantLoad()>Math.max(1,remaining.length)) { toast('DROP OR COMPLETE A RESEARCH GRANT BEFORE REDUCING THE SCIENCE TEAM'); return; }
  ...
  function chooseVessel(id) {
    ...
    const preview=crewPreviewForVessel(next), gate=gateStatus(next,preview.kept), credit=vesselTradeInValue(), due=Math.max(0,next.price-credit);
  ...
  function acceptOffer(id) {
    ...
    state.targets.forEach(item=>item.selected=false); offer.selected=true; state.targets.push(offer);
  ...
  function dropGrant(id) {
    const grant=state.targets.find(item=>item.id===id&&(item.kind==='grant'||item.kind==='contract'));
    if (!grant||activeOperation?.targetId===id) return;
    if (grant.deploymentId) { toast('THIS RETURN VISIT IS REQUIRED TO RECOVER DEPLOYED EQUIPMENT'); return; }
    state.targets=state.targets.filter(item=>item.id!==id); state.droppedGrantTemplates.push(grant.templateId);
    if (state.navigation?.id===id) state.navigation=null;
    if (state.lastTargetContext?.id===id) state.lastTargetContext=null;
    addLog(`Research grant dropped: ${grant.title}.`); generateOffers(state.port,{fresh:true}); toast('RESEARCH GRANT DROPPED'); changed();
  }
  ...
  function openCharacterSetup() {
    ...
    const modal=root.querySelector('#arx-character-modal'), starterIds=['coastal-oceanography','physical','coastal-ecology','sea-ice-physics','atmosphere','marine-mammals'];
    modal.innerHTML=`<div class="arx-modal-card arx-character-card"><small>EXPEDITION SETUP</small><h2>Choose your Chief Scientist</h2><p>You are permanently part of the crew. Your specialty determines the first research programs you can lead. You occupy a permanent berth and draw the same daily salary as any other graduate scientist; you cannot be hired or released.</p><div class="arx-avatar-grid">${PLAYER_AVATARS.map(avatar=>`<button class="${characterDraft.avatar===avatar.id?'selected':''}" data-arx-action="choose-avatar" data-id="${avatar.id}"><img src="${escapeHtml(avatar.src)}" alt="${escapeHtml(avatar.label)}"><span>${escapeHtml(avatar.label)}</span></button>`).join('')}</div><label class="arx-specialty-select"><span>STARTING SPECIALIZATION</span><select data-arx-character-specialty>${starterIds.map(id=>`<option value="${id}" ${characterDraft.specialty===id?'selected':''}>${escapeHtml(specialtyById[id].name)}</option>`).join('')}</select><small>${escapeHtml(specialtyById[characterDraft.specialty]?.description||'')}</small></label><div class="arx-character-summary"><b>You</b><span>Chief Scientist · Graduate career stage · permanent expedition member</span></div><button data-arx-action="confirm-character">BEGIN IN LONGYEARBYEN</button></div>`;
    modal.classList.add('open'); return true;
  }
  function confirmCharacter() {
    const modal=root.querySelector('#arx-character-modal'), select=modal.querySelector('[data-arx-character-specialty]');
    characterDraft.specialty=select?.value||characterDraft.specialty;
    const avatar=PLAYER_AVATARS.find(item=>item.id===characterDraft.avatar)||PLAYER_AVATARS[0], player=state.scientists.find(item=>item.isPlayer)||state.scientists[0];
    Object.assign(player,{id:'player',name:'You',role:'Chief Scientist',specialty:characterDraft.specialty,career:'grad',portrait:avatar.src,isPlayer:true,hiredAt:-1});
    state.playerConfigured=true; modal.classList.remove('open'); addLog(`You joined as Chief Scientist · ${specialtyById[player.specialty]?.name||player.specialty}.`); changed({port:false}); callbacks.onCharacterReady?.(getVesselModifiers());
  }
  ...
  function handleAction(event) {
    ...
    else if (action==='choose-avatar') { characterDraft.specialty=root.querySelector('[data-arx-character-specialty]')?.value||characterDraft.specialty; characterDraft.avatar=id; openCharacterSetup(); }
    ...
    else if (action==='accept-professor-grant'&&state.remoteOffer) { state.targets.forEach(item=>item.selected=false);state.remoteOffer.selected=true;state.targets.push(state.remoteOffer);addLog(`Professor-originated grant accepted: ${state.remoteOffer.title}.`);state.remoteOffer=null;root.querySelector('#arx-target-modal').classList.remove('open');renderSidebar();changed(); }
    ...
    else if (action==='open-vessel-port'&&state.port) { root.querySelector('#arx-vessel-modal').classList.remove('open'); activePortTab='vessel'; renderPort(); }
  ...
  function selectTarget(id) { state.targets.forEach(item=>item.selected=item.id===id); renderSidebar(); }
