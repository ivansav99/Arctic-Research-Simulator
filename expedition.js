(() => {
  ...
  function buildTarget(template, origin, rng, kind='grant') {
    ...
    const target={
      ...
      anywhere:!!template.anywhere, stationDelivery:!!template.stationDelivery, berthReserve:template.berthReserve||0, shore:!!template.shore, glacier:!!template.glacier, terrestrial:!!template.terrestrial, fjordPreferred:!!template.fjordPreferred, siteName:point.siteName||null, status:'active', kind, selected:false,
  ...
  function enterPort(port,options={}) {
    ...
    state.port={name:port.name,lat:port.lat,lon:port.lon,id:incomingId,countryCode:port.countryCode||null};
