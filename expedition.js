(() => {
  ...
  function openDevConsole() {
    ...
    const modal=root.querySelector('#arx-dev-modal'),player=playerScientist();
    modal.innerHTML=`<div class="arx-modal-card arx-dev-card"><button class="arx-close" data-arx-action="close-dev">×</button><small>TEMPORARY DEVELOPMENT TOOL</small><h2>Test progression state</h2><p>Jump directly to a vessel and Chief Scientist career stage. Applying a test state also tops up stores and provides test cash so later-game systems can be inspected immediately.</p><label><span>VESSEL</span><select data-arx-dev-vessel>${Object.values(VESSELS).map(ship=>`<option value="${ship.id}" ${ship.id===state.currentVessel?'selected':''}>${escapeHtml(ship.name)}</option>`).join('')}</select></label><label><span>CAREER STAGE</span><select data-arx-dev-career>${Object.values(CAREERS).map(career=>`<option value="${career.id}" ${career.id===player?.career?'selected':''}>${escapeHtml(career.name)}</option>`).join('')}</select></label><div class="arx-dev-warning">TEST MODE can alter the current local save. Do not use this state as a production playthrough.</div><button data-arx-action="apply-dev-state">APPLY TEST STATE</button></div>`;
    modal.classList.add('open');
  }
  function applyDevState() {
    const modal=root.querySelector('#arx-dev-modal'),vesselId=modal.querySelector('[data-arx-dev-vessel]')?.value,careerId=modal.querySelector('[data-arx-dev-career]')?.value;
    if(!VESSELS[vesselId]||!CAREERS[careerId])return;
    ...
    const ship=VESSELS[vesselId]; state.installedEquipment=state.installedEquipment.filter(id=>equipmentPossibleOnShip(EQUIPMENT[id],ship)); while(!equipmentFits(ship)&&state.installedEquipment.length)state.installedEquipment.pop();
    state.supplies=ship.supplyCapacity; callbacks.setResources?.({fuel:100,food:100}); recordScientist(player); addLog(`TEST MODE · ${CAREERS[careerId].name} aboard ${ship.name}.`);
    modal.classList.remove('open'); changed(); toast(`TEST STATE · ${ship.name.toUpperCase()} · ${CAREERS[careerId].short}`); callbacks.onStateChange?.();
  }
