(() => {
  ...
  function researchEnvironment(weather=currentWeather()){
    ...
    const ice=iceTypeAt(state.x,state.y),iceThickness=(ice==='packed'||ice==='cracked')?iceThicknessAt(state.x,state.y):0,packEdge=packIceEdge(position.lon),iceEdge=ice==='marginal'||Math.abs(position.lat-packEdge)<2.8;return{position,ice,iceThickness,iceEdge,location:locationName(position.lat,position.lon),weather,coastDistanceKm,coastal:coastDistanceKm<=85,fjord:fjordScore>=.48,fjordScore};
  }
  ...
  function iceNavigationProfileAt(x,y,vessel=vesselModifiers()){
    const type=iceTypeAt(x,y),thickness=(type==='packed'||type==='cracked')?iceThicknessAt(x,y):0,id=vesselIceId(vessel),cracked=type==='cracked';
    const result=(allowed,speedFactor=0,ramming=false,reason='')=>({allowed,speedFactor,ramming,reason,type,thickness,id,iceLabel:type==='open'?'OPEN WATER':type==='marginal'?'MARGINAL ICE':type==='fast'?'FAST ICE':`${thickness} M ${cracked?'FRACTURED':'PACK'} ICE`});
    if(type==='open')return result(true,1,false);
    if(type==='fast')return result(false,0,false,'FAST ICE · IMPASSABLE');
    if(['fishing','trawler','coastal'].includes(id))return result(false,0,false,'MARGINAL ICE · GLOBAL-CLASS R/V OR ICEBREAKER REQUIRED');
    if(id==='global')return type==='marginal'?result(true,1/3,false):result(false,0,false,'PACK ICE · ICEBREAKER REQUIRED');
    if(id==='icebreaker'){
      if(type==='marginal')return result(true,.6,false);
      if(thickness===1)return cracked?result(true,.3,true):result(true,.1,true);
      return result(false,0,false,`${thickness} M ICE · EXCEEDS BASIC ICEBREAKER CAPABILITY`);
    }
    if(id==='nuclear'){
      if(type==='marginal')return result(true,1,false);
      if(thickness===1)return result(true,.6,true);
      if(thickness===2)return result(true,cracked?.6:.3,true);
      if(thickness===3&&cracked)return result(true,.3,true);
      return result(false,0,false,thickness>=4?'4 M ICE · IMPASSABLE':'3 M UNFRACTURED PACK · IMPASSABLE');
    }
    return result(false,0,false,'SEA ICE · VESSEL NOT ICE-CAPABLE');
  }
  function iceStatusText(profile=iceNavigationProfileAt(state.x,state.y),ramming=state.ramming){if(state.frozen)return`FROZEN IN · ${profile.iceLabel}`;if(ramming)return`RAMMING · ${profile.iceLabel}`;return profile.iceLabel;}
  ...
  function portIceInfo(city){const center=polar(city.lat,city.lon);let sample=null;for(let radius=3;radius<=64&&!sample;radius+=3)for(let i=0;i<48;i++){const a=i*Math.PI/24,x=center.x+Math.cos(a)*radius,y=center.y+Math.sin(a)*radius,pos=unpolar(x,y);if(pos.lat<MIN_LAT||isBlocked(x,y))continue;sample={x,y};break;}if(!sample)return{frozen:true,iceLabel:'NO SEA APPROACH'};const type=naturalIceTypeAt(sample.x,sample.y),thickness=(type==='packed'||type==='cracked')?iceThicknessAt(sample.x,sample.y):0,frozen=type==='fast'||type==='packed'||type==='cracked';return{frozen,iceLabel:type==='open'?'OPEN WATER':type==='marginal'?'MARGINAL ICE':type==='fast'?'FAST ICE':`${thickness} M ${type==='cracked'?'FRACTURED':'PACK'} ICE`};}
  function getRelocationPorts(){return cityLabels.map(city=>({...city,...portIceInfo(city)}));}
  function relocateToPort(id){const city=cityLabels.find(item=>item.id===id);if(!city)return false;const ice=portIceInfo(city);if(ice.frozen)return false;const approach=findPortApproach(city);if(!approach)return false;state.x=approach.x;state.y=approach.y;state.tx=approach.x;state.ty=approach.y;state.moving=false;state.commandActive=false;state.ramming=false;state.ramClock=0;state.frozen=false;state.portDestination=null;state.targetOnLand=false;currentPortCity=city;state.dockedPort=city.name;enterPort(city);analytics.track('home_port_relocated',{port_name:city.name||'',port_country:city.country||'',cost:10000});return true;}
  ...
  function update(dt){
    ...
    state.food=Math.max(0,state.food-elapsedDays*100/vessel.foodEnduranceDays);research?.tickDays?.(elapsedDays,{position:unpolar(state.x,state.y),source:'sailing'});
    researchOpportunityClock+=elapsedDays;const opportunityEnv=researchEnvironment(weather),opportunityInterval=opportunityEnv.iceThickness>=3?.4:opportunityEnv.iceThickness>=2?.55:opportunityEnv.iceThickness>=1?.7:opportunityEnv.fjord?.7:opportunityEnv.iceEdge?.9:opportunityEnv.coastal?1.05:1.8;
    if(researchOpportunityClock>=opportunityInterval){researchOpportunityClock=0;research?.maybeSpawnOpportunity?.(opportunityEnv);}
    if(state.food<=0){ui.foodValue.textContent='0%';ui.foodLevel.style.width='0%';endGame('OUT OF FOOD','The expedition has exhausted its provisions. Return to port before supplies run out.');return;}
    let currentProfile=iceNavigationProfileAt(state.x,state.y,vessel),currentIce=currentProfile.type;
    if(!state.frozen&&!currentProfile.allowed){freezeIn();currentProfile=iceNavigationProfileAt(state.x,state.y,vessel);currentIce=currentProfile.type;}
    ...
    if(state.frozen){state.moving=false;ui.speed.textContent='0.0 KN';}
    else if(!vessel.nuclearFuel&&state.fuel<=0){ui.fuelValue.textContent='0%';ui.fuelLevel.style.width='0%';endGame('OUT OF FUEL','The vessel is stranded and the expedition cannot continue. Plan a shorter route between ports.');return;}
    else{
      ...
      let commanded=state.commandActive&&dist>arrivalRadius;const shouldRam=commanded&&(currentProfile.ramming||aheadProfile.ramming);if(shouldRam&&!state.ramming)state.ramClock=0;state.ramming=shouldRam;
      const driveProfile=state.ramming?(currentProfile.ramming?currentProfile:aheadProfile):currentProfile,normalCruise=Math.max(vessel.cruiseKnots*KNOT_TO_WORLD_SPEED,Math.min(vessel.maxKnots*KNOT_TO_WORLD_SPEED,dist*1.1+30));
      ...
      if(commanded&&state.ramming)cruise=ramCycleSpeed(dt,normalCruise*driveProfile.speedFactor);
