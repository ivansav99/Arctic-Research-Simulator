(() => {
  ...
  const sound=(()=>{
    ...
    const visibleAnimalSamples=()=>{const choices=[];try{forEachWildlifeVisual((entity,species,category,w)=>{const key=String(species||'').toUpperCase();if(!ANIMAL_SAMPLE_SOURCES[key])return;const p=worldToScreen(w.x,w.y);if(p.x<0||p.x>width||p.y<70||p.y>height)return;choices.push(key);});}catch(e){}return choices;};
