(() => {
  ...
  const compass=document.querySelector('.compass'),compassNorth=compass.querySelector('span'),compassNeedle=compass.querySelector('i'),minimapPanel=document.getElementById('minimap-panel'),minimapClose=document.getElementById('minimap-close');
  ...
  const isBlocked=(x,y)=>glacierAt(x,y)||(isLand(x,y)&&!riverAt(x,y));
  ...
  const GAME_VERSION='expedition-22p-glaciers',SAVE_VERSION=1;
  ...
  function refreshMenu(){
    ...
    const validAuto=!!auto?.research?.playerConfigured;
  ...
  function findPortApproach(city){
    ...
    for(let radius=3;radius<=420;radius+=3)for(let i=0;i<72;i++){
      const a=i*Math.PI/36,x=center.x+Math.cos(a)*radius,y=center.y+Math.sin(a)*radius,pos=unpolar(x,y);if(pos.lat<MIN_LAT||isBlocked(x,y))continue;const profile=iceNavigationProfileAt(x,y);if(!profile.allowed)continue;
  ...
  function terrainSeasonCanvas(tile){
    if(!tile.maskReady)return null;const winter=iceGrowth(),bucket=Math.round(winter*16);if(tile.seasonCanvas&&tile.seasonBucket===bucket&&tile.seasonSurfaceGeneration===surfaceElevationGeneration)return tile.seasonCanvas;const m=tile.maskSize,c=tile.seasonCanvas||document.createElement('canvas');c.width=c.height=m;const cc=c.getContext('2d'),image=cc.createImageData(m,m),out=image.data;
    for(let py=0;py<m;py++)for(let px=0;px<m;px++){const j=py*m+px;if(!tile.landMask[j])continue;const x=tile.minX+(px+.5)/m*TERRAIN_TILE_KM,y=tile.minY+(py+.5)/m*TERRAIN_TILE_KM,lat=unpolar(x,y).lat,high=surfaceElevationNormAt(x,y)??tile.heightProxy[j]/255;
      ...
      const mix=white/(white+green+.0001),i=j*4,snowGrey=Math.round(180+70*Math.sqrt(high));out[i]=Math.round(66+(snowGrey-66)*mix);out[i+1]=Math.round(132+(Math.min(253,snowGrey+4)-132)*mix);out[i+2]=Math.round(76+(Math.min(255,snowGrey+9)-76)*mix);out[i+3]=Math.round(Math.min(.92,a)*255);
    }cc.putImageData(image,0,0);tile.seasonCanvas=c;tile.seasonBucket=bucket;tile.seasonSurfaceGeneration=surfaceElevationGeneration;return c;
  }
  ...
  function update(dt){
    ...
    researchOpportunityClock+=elapsedDays;const opportunityInterval=opportunityEnv.ramming?.08:opportunityEnv.iceThickness>=3?.09:opportunityEnv.iceThickness>=2?.12:opportunityEnv.iceThickness>=1?.16:opportunityEnv.iceEdge?.24:opportunityEnv.fjord?.7:opportunityEnv.coastal?1.05:1.8;
    ...
    if(state.frozen){state.moving=false;ui.speed.textContent='0.0 KN';}
      ...
      else{state.x=nx;state.y=ny;const breakingPass=vesselCanBreakNaturalIceAt(state.x,state.y,vessel)||vesselCanBreakNaturalIceAt(fromX,fromY,vessel);if(breakingPass)carveIcebreakerTrack(fromX,fromY,state.x,state.y,vessel);if(commanded){state.travelled+=groundStep;const target=Math.atan2(dy,dx)+Math.PI/2;let da=((target-state.angle+Math.PI*3)%(Math.PI*2))-Math.PI;{const turnRate=dist<Math.max(30,arrivalRadius*8)?9:4.8;state.angle+=da*Math.min(1,dt*turnRate);state.moving=through>.02;ui.speed.textContent=through<.02?'0.0 KN':(through/KNOT_TO_WORLD_SPEED).toFixed(1)+(state.ramming?' KN ICE':' KN');}}else{const driftKnots=Math.hypot(groundX,groundY)/KNOT_TO_WORLD_SPEED;state.moving=false;state.commandActive=false;state.ramming=false;ui.speed.textContent=driftKnots<.05?'0.0 KN':driftKnots.toFixed(1)+' KN DRIFT';if(state.portDestination)enterPort(state.portDestination);}}
    }
