(() => {
  ...
  const sound=(()=>{
    ...
    const update=paused=>{if(!ac)return;const now=performance.now(),day=((state.seasonDay%365)+365)%365,season=day<91?'autumn':day<213?'winter':day<274?'spring':'summer';if(waveGain)waveGain.gain.setTargetAtTime((!paused&&state.commandActive&&state.moving&&!state.ramming)?0.045:0,ac.currentTime,.22);if(ambientPadGain)ambientPadGain.gain.setTargetAtTime(0,ac.currentTime,.2);if(!paused&&ac.state==='running'){if(!nextAnimal)nextAnimal=now+7000+Math.random()*7000;if(now>=nextAnimal){const species=nearbyAnimal();if(species&&playAnimal(species)){lastAnimal=now;nextAnimal=now+22000+Math.random()*22000;}else nextAnimal=now+6000+Math.random()*5000;}}if(!paused&&state.ramming&&now-lastCrack>850){lastCrack=now;play('ice');}};
