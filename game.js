(() => {
  ...
  const sound=(()=>{
    let ac=null,waveSource=null,waveGain=null,ambientWindGain=null,ambientPadGain=null,ambientPadOsc=[],lastCrack=0,lastAnimal=0,unlockChimed=false,unlockPromise=null,nextBird=0,nextAnimal=0;
    // SEASONAL ARCTIC AMBIENCE: quiet by default; a low tonal bed and sparse seasonal details.
    const setupAmbience=c=>{if(ambientPadGain)return;const padFilter=c.createBiquadFilter();padFilter.type='lowpass';padFilter.frequency.value=720;ambientPadGain=c.createGain();ambientPadGain.gain.value=.0001;padFilter.connect(ambientPadGain).connect(c.destination);for(const freq of[196,293.7]){const o=c.createOscillator(),g=c.createGain();o.type='sine';o.frequency.value=freq;g.gain.value=freq<250?.55:.24;o.connect(g).connect(padFilter);o.start();ambientPadOsc.push(o);}};
