(() => {
  ...
  const sound=(()=>{
    let ac=null,waveSource=null,waveGain=null,ambientWindGain=null,ambientPadGain=null,ambientPadOsc=[],lastCrack=0,lastAnimal=0,unlockChimed=false,unlockPromise=null,nextBird=0,nextBell=0,nextAnimal=0;
    ...
    const play=(type)=>{const c=ensure();if(!c)return;const run=()=>{switch(type){case'cash':case'cash-in':case'cash-out':burst(.035,.045,1500,5200);tone(1480,.075,.055,.015,'square');tone(2080,.16,.045,.045,'sine');break;case'data':tone(980,.07,.035,0,'square');tone(1260,.06,.025,.07,'square');break;case'paper-accepted':for(let i=0;i<7;i++)burst(.35,.035,500,3500,i*.07);tone(523,.5,.07,.05);tone(659,.5,.06,.12);tone(784,.6,.06,.2);break;case'paper-rejected':tone(430,.5,.06);tone(350,.6,.055,.15);tone(270,.75,.05,.3);break;case'port':for(const offset of[0,.86]){burst(.09,offset? .085:.11,420,3200,offset);tone(285,2.4,offset? .09:.12,offset,'sine');tone(438,2.05,offset? .056:.075,offset+.01,'sine');tone(672,1.65,offset? .04:.052,offset+.015,'sine');tone(910,1.1,offset? .024:.032,offset+.02,'sine');}break;case'depart':ferryHorn(0,2.55);break;case'ice':burst(.48,.12,110,1750);burst(.24,.085,380,3200,.09);burst(.18,.065,700,4200,.23);tone(92,.38,.055,0,'sawtooth');tone(138,.22,.035,.12,'square');break;case'whale':tone(145,.9,.045,0,'sine');tone(105,1.2,.035,.4,'sine');break;case'bird':tone(1800,.08,.03);tone(2350,.08,.025,.1);tone(1600,.09,.02,.2);break;case'seal':burst(.16,.035,500,1700);tone(330,.16,.025,.04,'square');break;case'walrus':burst(.28,.026,110,620);tone(148,.34,.030,.02,'triangle');tone(118,.24,.020,.23,'sine');break;case'mammal':tone(520,.12,.025,0,'square');tone(390,.15,.02,.13,'square');break;case'fish':tone(1150,.025,.012);tone(900,.025,.01,.05);break;}};if(c.state!=='running'){Promise.resolve(c.resume()).then(()=>{if(c.state==='running')run();}).catch(()=>{});return;}run();}
    ...
    const update=paused=>{if(!ac)return;const now=performance.now(),day=((state.seasonDay%365)+365)%365,season=day<91?'autumn':day<213?'winter':day<274?'spring':'summer';if(waveGain)waveGain.gain.setTargetAtTime((!paused&&state.commandActive&&state.moving&&!state.ramming)?0.045:0,ac.currentTime,.22);if(ambientPadGain){const pad=paused?0:{autumn:.0120,winter:.0086,spring:.0136,summer:.0116}[season];ambientPadGain.gain.setTargetAtTime(pad,ac.currentTime,1.3);}if(!paused&&ac.state==='running'){if(!nextBird)nextBird=now+9000+Math.random()*8000;if(!nextBell)nextBell=now+18000+Math.random()*14000;if(season==='summer'&&now>=nextBird){tone(1780,.10,.008,0,'sine');tone(2240,.08,.007,.11,'sine');tone(1960,.09,.006,.23,'sine');nextBird=now+13000+Math.random()*18000;}else if(season!=='summer'&&now>=nextBird)nextBird=now+12000;if(now>=nextBell){const base={autumn:196,winter:146.8,spring:220,summer:246.9}[season];tone(base,1.7,.0045,0,'sine');tone(base*1.5,1.3,.003,.24,'sine');nextBell=now+26000+Math.random()*26000;}if(!nextAnimal)nextAnimal=now+22000+Math.random()*26000;if(now>=nextAnimal){const animal=nearbyAnimal(),type=animal==='walrus'?'walrus':animal;if(['whale','seal','walrus','bird'].includes(type)){play(type);lastAnimal=now;nextAnimal=now+30000+Math.random()*45000;}else nextAnimal=now+12000+Math.random()*10000;}}if(!paused&&state.ramming&&now-lastCrack>850){lastCrack=now;play('ice');}};
  ...
  function iceRuleKey(type,thickness=0){if(type==='marginal')return type;if(type==='packed'||type==='cracked')return`${type}${thickness}`;return'open';}
  ...
  function vesselCanBreakNaturalIceAt(x,y,vessel=vesselModifiers()){
    const type=naturalIceTypeAt(x,y),thickness=(type==='packed'||type==='cracked')?iceThicknessAt(x,y):0;
  ...
  function drawIceThicknessAndCracks(){
    ...
    for(const[level,texture,overlay,alpha,tilePx]of[[2,iceTextures.pack,'rgba(177,211,224,.13)',.9,350],[3,iceTextures.dense,'rgba(238,247,247,.28)',.92,360],[4,iceTextures.dark,'rgba(37,83,122,.25)',.82,380]]){
  ...
  function drawFishSchools(){
    ...
    for(const school of fishSchools){
      ...
      const ice=iceTypeAt(school.x,school.y);if(ice==='packed'||ice==='cracked'||isLand(school.x,school.y)||!wildlifeClearOfPorts(school.x,school.y))continue;
  ...
  function researchEnvironment(weather=currentWeather()){
    ...
    if(!researchEnvCache||now-researchEnvCacheAt>900||moved>7){
      ...
      const ice=iceTypeAt(state.x,state.y),iceThickness=(ice==='packed'||ice==='cracked')?iceThicknessAt(state.x,state.y):0,packEdge=packIceEdge(position.lon),iceEdge=ice==='marginal'||Math.abs(position.lat-packEdge)<2.8;
  ...
  function nearbyResearchTargetAt(clientX,clientY){let match=null,best=36;for(const target of researchTargets()){const item=researchTargetWorld(target),hit=Math.hypot(item.p.x-clientX,item.p.y-clientY);if(hit<best){match=item;best=hit;}}return match;}
  ...
  function updateResearchNavigation(){
    if(pendingResearchTargetId){const pending=researchTargets().find(item=>item.id===pendingResearchTargetId);if(!pending){pendingResearchTargetId=null;pendingResearchArrival=null;}else if(pendingResearchArrival&&!research?.isBusy?.()){const remaining=Math.hypot(state.x-pendingResearchArrival.x,state.y-pendingResearchArrival.y),actual=researchTargetWorld(pending).distance;if(remaining<=RESEARCH_INTERACTION_KM||actual<=RESEARCH_INTERACTION_KM){pendingResearchTargetId=null;pendingResearchArrival=null;state.tx=state.x;state.ty=state.y;state.commandActive=false;state.moving=false;state.ramming=false;research?.selectTarget?.(pending.id);research?.openTarget?.(pending.id,{distanceKm:Math.min(actual,remaining),atSite:true,target:pending});}}}
  ...
  function markerSurfaceTone(x,y){const ice=iceTypeAt(x,y);return isLand(x,y)||ice==='packed'||ice==='cracked'||ice==='marginal'?'light':'dark';}
  ...
  function updateWildlife(dt){for(const whale of whales){const habitat=iceTypeAt(whale.x,whale.y);if(isLand(whale.x,whale.y)||!wildlifeClearOfPorts(whale.x,whale.y)||coastDistance(whale.x,whale.y,10)<2||habitat==='packed'||habitat==='cracked'){const safe=safeWhaleWater(whale.x,whale.y,whale.phase);whale.x=safe.x;whale.y=safe.y;}const desired=chooseWhaleCourse(whale),turn=((desired-whale.angle+Math.PI*3)%(Math.PI*2))-Math.PI;whale.angle+=Math.max(-.055,Math.min(.055,turn))+Math.sin(state.seasonDay*.8+whale.phase)*.002;const flow=currentAt(whale.x,whale.y,false),swim=5.5,vx=Math.cos(whale.angle)*swim+flow.vx*.22,vy=Math.sin(whale.angle)*swim+flow.vy*.22,nx=whale.x+vx*dt,ny=whale.y+vy*dt,type=iceTypeAt(nx,ny),pos=unpolar(nx,ny);if(pos.lat<MIN_LAT||isLand(nx,ny)||!wildlifeClearOfPorts(nx,ny)||type==='packed'||type==='cracked'){const safe=safeWhaleWater(whale.x,whale.y,whale.phase);whale.x=safe.x;whale.y=safe.y;whale.angle=chooseWhaleCourse(whale);}else{whale.x=nx;whale.y=ny;}}}
  const npcActive=npc=>!npc.seasonal||summerWildlifeVisible();
  function npcAllowedAt(npc,x,y){const pos=unpolar(x,y);if(pos.lat<MIN_LAT+.03||isBlocked(x,y))return false;const ice=iceTypeAt(x,y);return ice!=='packed'&&ice!=='cracked'&&(!npc.avoidMarginal||ice!=='marginal');}
  ...
  function fracturedIceAhead(x,y,angle,distance=60){for(let d=4;d<=distance;d+=4){const cx=x+Math.cos(angle)*d,cy=y+Math.sin(angle)*d;if(isCrackedIceAt(cx,cy))return true;const ice=iceTypeAt(cx,cy);if(isBlocked(cx,cy)||ice==='packed')return false;}return false;}
  ...
  function frame(now){
    ...
    if(!paused){
      ...
      if(!paused){const margin=60/scale,minX=state.x-width/scale/2-margin,maxX=state.x+width/scale/2+margin,minY=state.y-height/scale/2-margin,maxY=state.y+height/scale/2+margin;floeUpdateAccumulator+=dt/zoomLevel;if(floeUpdateAccumulator>=.035){updateFloes(Math.min(.18,floeUpdateAccumulator),minX,maxX,minY,maxY);floeUpdateAccumulator=0;}updateWakeFloes(dt/zoomLevel);updateWildlifeEncounters(dt);wildlifeMotionAccumulator+=dt/zoomLevel;if(wildlifeMotionAccumulator>=.12){const wildlifeStep=Math.min(.25,wildlifeMotionAccumulator);wildlifeMotionAccumulator=0;updateFishSchools(wildlifeStep);updateWildlife(wildlifeStep);}updateNpcVessels(dt/zoomLevel);}
