(() => {
  ...
  function departWithCheck(proceed){const depart=()=>{if(currentPortCity){sound.play('depart');research?.leavePort?.();state.dockedPort=null;currentPortCity=null;}proceed();};const warned=!!currentPortCity&&!!research?.confirmDeparture?.({fuel:state.fuel,food:state.food},depart);if(!warned)depart();}
