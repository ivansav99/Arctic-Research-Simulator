(() => {
  ...
  const CAREERS = {
    grad: {id:'grad', name:'Graduate Student', short:'GRAD', level:1, minCitations:0, salary:300, quality:.88, productivity:1, color:'#8ef0cf', next:'postdoc', promotion:{papers:2,missions:10}},
    postdoc: {id:'postdoc', name:'Postdoctoral Researcher', short:'POSTDOC', level:2, minCitations:25, salary:800, quality:1.12, productivity:1.35, color:'#7dd3fc', next:'professor', promotion:{papers:10,missions:50}},
    professor: {id:'professor', name:'Professor', short:'PROF', level:3, minCitations:100, salary:1900, quality:1.42, productivity:1.8, color:'#f6d365', next:null, promotion:null}
  };
  ...
  const VESSELS = {
    fishing: {
      ...
      slots:{light:4, medium:0, heavy:0}, helidecks:0, minZoom:1.8,
      ...
      description:'A paid-off local platform with a hull echosounder, three berths and only light portable science mounts.'
    },
    trawler: {
      ...
      standardEquipment:['hull-echosounder'], upgradeGate:null,
      description:'A paid-off-style working trawler refit with five berths, triple the starter stores and twice the light-equipment deck capacity.'
    },
    coastal: {
      id:'coastal', name:'Coastal-Class Research Vessel', shipName:'R/V Kongsfjord', className:'COASTAL CLASS', price:750000, berths:10,
      slots:{light:8, medium:5, heavy:0}, helidecks:0, minZoom:1.1,
      ...
      standardEquipment:['hull-echosounder'], upgradeGate:{career:'postdoc', count:1, label:'At least one postdoc or professor aboard'},
      description:'A regional vessel with ten berths and room for several medium winches, labs and communications systems.'
    },
    global: {
      id:'global', name:'Global-Class Research Vessel', shipName:'R/V Aurora', className:'GLOBAL CLASS', price:8000000, berths:20,
      ...
      standardEquipment:['hull-echosounder'], upgradeGate:{career:'professor', count:2, label:'Two professors aboard'},
    ...
    icebreaker: {
      ...
      standardEquipment:['hull-echosounder'], upgradeGate:{career:'professor', count:3, label:'Three professors aboard'},
    ...
    nuclear: {
      ...
      standardEquipment:['hull-echosounder'], upgradeGate:{career:'professor', count:4, label:'Four professors aboard'},
  ...
  const EQUIPMENT = {
    ...
    'argo-float': equipment({id:'argo-float', name:'Polar Profiling Float', price:38000, consumable:true, units:1, tier:2, specialties:['physical','biogeochemistry'], description:'One autonomous profiling float that cycles through the upper 2,000 m and telemeters data.', specs:['One multi-year autonomous float','10-day profiling cycle','Ice-sensing polar firmware'], media:MEDIA.argo}),
    'ice-tethered-profiler': equipment({id:'ice-tethered-profiler', name:'Ice-Tethered Profiler Package', price:165000, consumable:true, units:1, tier:2, specialties:['physical','sea-ice-physics','moorings'], description:'One autonomous ice-floe surface package, tether and crawling ocean profiler.', specs:['Repeated under-ice profiles','Moves with its host floe','Possible early loss when ice fails'], media:MEDIA.ice}),
    ...
    'axctd-pack': equipment({id:'axctd-pack', name:'AXCTD Air-Drop Case', price:55000, consumable:true, units:6, tier:3, requiresEquipment:['manned-helicopter'], specialties:['physical','naval-acoustics'], description:'Six airborne expendable conductivity-temperature-depth probes.', specs:['6 air-dropped profiles','Requires the research helicopter detachment','Temperature, salinity and sound speed'], media:MEDIA.xbt})
  };
  ...
  const state = {
    money:185000, citations:0, data:0, supplies:35, currentVessel:'fishing', ownedVessels:['fishing'],
    ...
    scientistRecords:{}, promotions:[], recentGrantTemplates:[], recentGrantSites:[], grantCooldowns:{}, grantMarketReady:{}, assistedByVessels:[], bridgeSupportNotice:null
  };
  ...
  function vesselMarketUnlock(id) {
    const needed={fishing:0,trawler:0,coastal:4,global:18,icebreaker:35,nuclear:55}[id] ?? Infinity;
    return state.completed.length>=needed;
  }
  ...
  function checkPromotions() {
    for (const scientist of state.scientists) {
      let career=CAREERS[scientist.career];
      while (career?.next && scientist.papers>=(career.promotion?.papers||Infinity) && scientist.missions>=(career.promotion?.missions||Infinity)) {
        scientist.career=career.next; career=CAREERS[scientist.career]; recordScientist(scientist);
        const message=scientist.career==='postdoc'
          ? 'Two papers and ten specialty missions completed the graduate program. Their equipment clearance, scientific ability and daily salary have been upgraded automatically.'
          : 'Ten publications and fifty specialty missions have earned promotion to professor. They can now lead heavy-instrument programs.';
        promotionQueue.push({name:scientist.name,career:scientist.career,papers:scientist.papers||0,missions:scientist.missions||0,message});
        addLog(`${scientist.name} promoted to ${career.name}.`);
      }
    }
  ...
  function equipmentPurchaseStatus(item,ship=vessel()) {
    ...
    const possible=equipmentPossibleOnShip(item,ship), capacity=item.consumable||item.builtIn||equipmentFits(ship,item.id);
    const crewReady=crewRequirementsMet(equipmentCrewRequirements(item)), prerequisites=(item.requiresEquipment||[]).every(id=>equipmentOperational(id));
    const support=crewReady&&prerequisites, affordable=state.money>=item.price;
    let reason='Ready to purchase';
    if (!possible) reason='Not supported by this vessel class';
    else if (!capacity) reason=`No available ${item.helideckUse?'helideck or ':''}${item.slotType||'science'} capacity`;
    else if (!crewReady) reason=`Current crew cannot operate this ${item.tier===3?'professor':item.tier===2?'postdoc':'graduate'}-level system`;
    else if (!prerequisites) reason=`Requires ${item.requiresEquipment.map(id=>EQUIPMENT[id]?.name||id).join(' + ')} aboard and operational`;
    else if (!affordable) reason='Insufficient funds';
    return {ready:possible&&capacity&&support&&affordable,possible,capacity,crewReady,prerequisites,support,affordable,reason};
  }
  ...
  function buildTarget(template, origin, rng, kind='grant') {
    ...
    const target={
      ...
      data:Math.max(1,Math.round(template.data*scale*vesselScale*crewScale)), reward:Math.round(template.reward*scale*(kind === 'opportunity' ? 1.6 : 2)),
      ...
      anywhere:!!template.anywhere, stationDelivery:!!template.stationDelivery, berthReserve:template.berthReserve||0, status:'active', kind, selected:false,
  ...
  function generateOffers(port,{fresh=false}={}) {
    if (!port) return;
    const portId=normalizedPortId(port), cycle=`${portId}:${state.portVisits}`;
    if (!fresh&&state.grantOfferCycle===cycle) return;
    state.grantOfferCycle=cycle;
    const rng=seeded(`${normalizedPortId(port)}-${state.portVisits}-grants-v4`), activeTemplates=new Set(activeGrants().map(item=>item.templateId));
    const available=TEMPLATES.filter(item => !item.weather && eligible(item) && !activeTemplates.has(item.id) && !(state.droppedGrantTemplates||[]).includes(item.id) && (item.unlockAfter||0)<=state.completed.length && (!item.onlyPorts||item.onlyPorts.includes(normalizedPortId(port))) && (state.grantCooldowns?.[`${portId}:${item.id}`]||0)<=state.elapsedDays);
    const shuffle=list=>{for(let i=list.length-1;i>0;i--){const j=Math.floor(rng()*(i+1));[list[i],list[j]]=[list[j],list[i]];}return list;};
    const recent=new Set(state.recentGrantTemplates||[]), unseen=available.filter(item=>!recent.has(item.id)), source=unseen.length>=2?unseen:available;
    const local=shuffle(source.filter(item=>item.tier==='local'));
    const regular=shuffle(source.filter(item=>item.tier!=='local'));
    const starter=['fishing','trawler'].includes(state.currentVessel), pool=starter&&state.completed.length<6?[...local,...regular]:starter?[...local.slice(0,1),...regular,...local.slice(1)]:[...regular,...local];
    if (starter&&state.completed.length===0) {
      const harbor=pool.find(item => item.id==='harbor-soundings');
      if (harbor) { pool.splice(pool.indexOf(harbor),1); pool.unshift(harbor); }
    }
    state.offers=[];
    const specialtyCount=new Set(state.scientists.map(item=>item.specialty)).size, offerLimit=Math.min(6,2+specialtyCount);
  ...
  function scientistCard(item,candidate=false) {
    const spec=specialtyById[item.specialty] || {name:item.specialty,description:''};
    const career=CAREERS[item.career], profile=profileFor(item), locked=candidate&&state.citations<career.minCitations, full=state.scientists.length>=vessel().berths;
    const capability=career.level===1?'LIGHT EQUIPMENT':career.level===2?'LIGHT + MEDIUM EQUIPMENT':'LIGHT + MEDIUM + HEAVY EQUIPMENT';
    const selected=candidate&&pendingCandidateId===item.id;
    let action;
    if (candidate) action=locked
      ? `<button disabled>${career.minCitations} citations required</button>`
      : full
        ? `<button data-arx-action="select-recruit" data-id="${item.id}">${selected?'SELECTED · CHOOSE A CREW MEMBER':'SELECT TO REPLACE CREW'}</button>`
        : `<button data-arx-action="hire" data-id="${item.id}">Hire · ${cash(career.salary)}/day</button>`;
    else if (item.isPlayer) action='<button disabled>Chief Scientist · permanent berth</button>';
    else if (pendingCandidateId&&full) action=`<button data-arx-action="replace-scientist" data-id="${item.id}">REPLACE WITH SELECTED RECRUIT</button>`;
    else action=`<button class="ghost" data-arx-action="release" data-id="${item.id}">Release at port</button>`;
    const promotion=career.promotion, progress=promotion?`Career progress · ${item.papers||0}/${promotion.papers} papers · ${item.missions||0}/${promotion.missions} specialty missions`:'Senior career stage';
  ...
  function gateStatus(ship,scientists=state.scientists) {
    const gate=ship.upgradeGate;
    if (!gate) return {ready:true,label:'No seniority gate'};
    const neededLevel=careerLevel(gate.career);
    const count=scientists.filter(item => careerLevel(item.career)>=neededLevel && (gate.career!=='professor'||item.career==='professor')).length;
    return {ready:count>=gate.count,label:`${gate.label} · ${count}/${gate.count}`};
  }
  ...
  function vesselTradeInValue(ship=vessel()) {
    const hull=Math.round((ship.price||0)*VESSEL_TRADE_IN_RATE);
  ...
  function vesselCard(item) {
    const active=state.currentVessel===item.id, unlocked=vesselMarketUnlock(item.id), forSale=vesselForSaleHere(item.id), preview=crewPreviewForVessel(item), gate=gateStatus(item,preview.kept);
    const blockedAsset=deployedTradeAsset(), credit=vesselTradeInValue(), due=Math.max(0,item.price-credit), grantBlocked=grantLoad()>Math.max(1,preview.kept.length);
    const fuel=item.nuclearFuel?'REACTOR · ∞':formatCapacity(item.fuelCapacity,'L'), unlockNeed={fishing:0,trawler:0,coastal:4,global:18,icebreaker:35,nuclear:55}[item.id]||0;
    const checks=[
      {ok:active||unlocked,text:active?'Current vessel':`Research record: ${state.completed.length}/${unlockNeed} completed missions`},
      {ok:active||forSale,text:forSale?'Sold at this port':item.id==='nuclear'?'Sold only in Murmansk':'Conventional vessels sold in Longyearbyen'},
      {ok:active||!blockedAsset,text:blockedAsset?`Recover ${EQUIPMENT[blockedAsset]?.name||'deployed equipment'}`:'No deployed trade-blocking equipment'},
      {ok:active||gate.ready,text:gate.label},
      {ok:active||!grantBlocked,text:grantBlocked?'Reduce active grants before downsizing crew':'Active grants fit retained crew'},
      {ok:active||state.money>=due,text:`Cash after trade credit: need ${cash(due)}`}
    ];
    const disabled=active||checks.some(check=>!check.ok), image=item.image||'assets/vessels/base-vessel.png';
    const reason=active?'CURRENT VESSEL':disabled?'REQUIREMENTS NOT MET':`TRADE IN & COMMISSION · ${cash(due)}`;
    return `<details class="arx-card arx-store-details ${active?'selected':''}" data-arx-store-details="vessel-${item.id}"><summary><span><b>${escapeHtml(item.shipName||item.name)}</b><small>${escapeHtml(item.name)} · ${item.className} · ${item.berths} BERTHS</small></span><em class="${disabled&&!active?'price-locked':''}">${item.price?cash(item.price):'PAID OFF'}</em></summary><div class="arx-detail-split"><figure class="arx-media compact"><img src="${escapeHtml(image)}" alt="Side view of ${escapeHtml(item.shipName||item.name)}"></figure><div><p>${escapeHtml(item.description)}</p><ul class="arx-spec-list arx-vessel-specs"><li>${item.cruiseKnots} kn cruise · ${item.maxKnots} kn maximum</li><li>${fuel} fuel · ${item.foodEnduranceDays} d provisions</li><li>${item.helidecks} helideck${item.helidecks===1?'':'s'}</li><li>${slotSummary(item,{light:0,medium:0,heavy:0})}</li><li>${item.berths} total berths</li><li>List price ${cash(item.price)} · current trade credit ${cash(credit)}</li>${preview.removed.length?`<li>${preview.removed.length} newest hire${preview.removed.length===1?'':'s'} would remain ashore</li>`:''}</ul><div class="arx-requirement-checklist">${checks.map(check=>`<div class="${check.ok?'ready':'missing'}"><i>${check.ok?'✓':'!'}</i><span>${escapeHtml(check.text)}</span></div>`).join('')}</div><button data-arx-action="vessel" data-id="${item.id}" ${disabled?'disabled':''}>${reason}</button></div></div></details>`;
  }
  ...
  function equipmentCard(item) {
    ...
    const priceLabel=item.consumable?`${cash(item.price)} / ${item.units} unit${item.units===1?'':'s'}`:item.builtIn?'STANDARD':cash(item.price);
    let actionLabel, disabled=false;
    if (item.builtIn) { actionLabel='Standard vessel equipment'; disabled=true; }
    else if (item.consumable) { actionLabel=purchase.ready?`Buy ${item.units} · ${cash(item.price)}`:purchase.reason; disabled=!purchase.ready; }
  ...
  function enterPort(port,options={}) {
    state.port={name:port.name,lat:port.lat,lon:port.lon,id:port.id||slug(port.name),countryCode:port.countryCode||null};
    if (!options.resume) {
      ...
      if (state.elapsedDays>=readyAt) { state.grantOfferCycle=null; generateOffers(state.port,{fresh:true}); state.grantMarketReady[portId]=state.elapsedDays+7; }
      else state.grantOfferCycle=`${portId}:${state.portVisits}`;
  ...
  function completionModal(target,title,body,stats=[]) {
    ...
    modal.innerHTML=`<div class="arx-modal-card arx-target-card arx-operation arx-complete"><small>FIELD PROGRAM UPDATE</small><h2>${escapeHtml(title)}</h2><p>${escapeHtml(body)}</p><div class="arx-operation-progress"><div><b>100%</b><span>${Math.round(target.lastOperationHours||operationWorkHours(target))} person-hours completed at this station</span></div><i><em style="width:100%"></em></i></div><ol>${steps.map((step,index)=>`<li class="done"><i>✓</i><b>${escapeHtml(step)}</b><span>Complete</span></li>`).join('')}</ol>${stats.length?`<div class="arx-chance">${stats.map(item=>`<span>${escapeHtml(item.label)}<b>${escapeHtml(item.value)}</b></span>`).join('')}</div>`:''}<button data-arx-action="acknowledge-research">OKAY</button></div>`;
    modal.classList.add('open');
  }
  ...
  function maybeSpawnOpportunity(payload={}) {
    ...
    if (state.targets.filter(item=>item.kind==='opportunity').length>=5) return null;
    ...
    const coastal=payload.fjord||payload.fjordScore>.38||payload.coastal||payload.coastDistanceKm<30, iceEdge=!!payload.iceEdge||payload.ice==='marginal';
    const weighted=possible.flatMap(template=>{let weight=1;if(coastal&&(template.coastal||template.fjordPreferred||template.tier==='local'))weight+=7;if(iceEdge&&template.iceAllowed)weight+=10;if(!coastal&&template.tier!=='local')weight+=2;return Array(weight).fill(template);});
  ...
  function publishPaper(automatic=false) {
    ...
    state.publishAttempts++; state.publicationCooldown=(accepted?12:4)*(connected?.75:1);
    ...
    if (accepted) {
      ...
      heading='Manuscript rejected'; message=`Reviewers requested revision. No data were lost: all ${new Intl.NumberFormat().format(state.data)} data remain available for the next submission.`;
  ...
  function hire(id) {
    ...
    if (state.citations<career.minCitations) return;
    ...
    generateOffers(state.port); addLog(`${item.name} joined as ${career.name}.`); changed();
  }
  function selectRecruit(id) {
    const item=state.candidates.find(candidate=>candidate.id===id); if (!item||state.citations<CAREERS[item.career].minCitations) return;
  ...
  function replaceScientist(outgoingId) {
    ...
    if (!incoming||!outgoing||outgoing.isPlayer||state.citations<CAREERS[incoming.career].minCitations) return;
    ...
    pendingCandidateId=null; generateOffers(state.port); addLog(`${incoming.name} replaced ${outgoing.name}; ${outgoing.name} remains available during this port call.`); changed();
  }
  function release(id) {
    ...
    generateOffers(state.port); addLog(`${item.name} left the expedition and remains available to rehire during this port call.`); changed();
  }
  function chooseVessel(id) {
    ...
    callbacks.onVesselChanged?.(getVesselModifiers()); generateOffers(state.port); changed();
  }
  function buyEquipment(id) {
    ...
    if (item.consumable) {
      adjustMoney(-item.price); state.inventory[id]=(state.inventory[id]||0)+(item.units||1); addLog(`${item.name} loaded.`);
    } else {
    ...
    generateOffers(state.port); changed();
  }
  function sellEquipment(id) {
    ...
    addLog(`${item.name} sold for ${cash(item.price*EQUIPMENT_RESALE_RATE)}.`); generateOffers(state.port); changed();
  }
  ...
  function dropGrant(id) {
    ...
    addLog(`Research grant dropped: ${grant.title}.`); generateOffers(state.port); toast('RESEARCH GRANT DROPPED'); changed();
  }
  ...
  function restoreCheckpoint(snapshot) {
    ...
    state.observed=state.observed||[]; state.observedIndividuals=state.observedIndividuals||[]; state.recentGrantTemplates=state.recentGrantTemplates||[]; state.recentGrantSites=state.recentGrantSites||[]; state.grantCooldowns=state.grantCooldowns||{}; state.grantMarketReady=state.grantMarketReady||{}; state.assistedByVessels=state.assistedByVessels||[]; state.bridgeSupportNotice=state.bridgeSupportNotice||null; state.elapsedDays=Number(state.elapsedDays)||0; state.playerConfigured=!!state.playerConfigured;
    ...
    state.offers=(state.offers||[]).filter(Boolean).slice(0,6);
    renderSidebar();
  }
